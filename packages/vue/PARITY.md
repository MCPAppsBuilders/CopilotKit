...
## Architectural decision: MCP Apps host -> shared framework-agnostic package

`MCPAppsActivityRenderer` no longer implements the app<->host protocol. Vue
consumes `@copilotkit/mcp-apps-renderer` - the same package react-core and
Angular use - so the renderer is a thin adapter: it owns the iframe
(create/mount/size/remove) and wires session hooks to Vue refs, while
`bindMcpApp` owns the protocol.

### Rule

- The hand-rolled JSON-RPC router, sandbox HTML builder, request queue and
  `PROTOCOL_VERSION` literal are deleted, not re-implemented per frontend.
- The bridge-free `/activity` entry is re-exported for activity registration;
  the bridge itself loads through a dynamic `import()` so a non-MCP app does not
  pay for it. Both the root entry and its subpaths stay `external` in
  `vite.config.ts` so that lazy boundary survives the build.
- Session, iframe and fetched resource are held in `shallowRef` (Vue reactivity
  boundary: they cross into the widget and proxies are not clone-safe).
- The bind watcher takes the container element as a source: an `immediate`
  watcher runs before the template ref is populated.

### Observable behavior changes vs the previous Vue host

1. Protocol version moves from the hardcoded `2025-06-18` to the ext-apps
   negotiated version; the host returns its own version rather than echoing the
   widget's.
2. `ui/initialize` is validated against the spec schema by the bridge, so a
   widget omitting required fields (e.g. `appCapabilities`) now fails initialize
   instead of receiving a lenient hand-rolled response.
3. `ui/open-link` follows one policy shared by all three frontends: a scheme
   denylist (`javascript:`, `data:`, `vbscript:`, `blob:`, `file:`) that keeps
   custom-scheme deep links working, plus two guarantees Vue did not have -
   embedded credentials refused, and `/path` style relative links resolved.
4. `ui/message` gains the `_meta.copilotkit` extension channel (top-level
   `role`/`followUp` still honored) and the issue #5819 thread-capture guard.
5. Tool input/result are pushed by the session itself for activities that live in
   the agent's message store, keyed on the activity `messageId`; the adapter only
   forwards content for activities rendered from an external messages list.
6. The iframe is created up front and kept across tool input/result updates: it is
   recreated only when the widget identity (`resourceUri`/`serverHash`/
   `serverId`), the agent, the host or the message id changes.
7. Queue guarantees are shared rather than local: a 30s busy-agent timeout, and
   teardown cancels the requests this widget owns instead of every request on its
   thread. Only _waiting_ work is cancelled - `runAgent` exposes no abort.
8. Content rejected by the schema is reported as RECOVERABLE (`onContentError`)
   and cleared when valid content resumes, distinct from a fatal session error,
   for as long as the exchange that produced it is still open.
9. A result that cannot be used once its exchange has settled retires the
   widget: the renderer renders nothing on `onRemoved`, recorded against the
   exchange identity so the same activity mounts again for a different
   resource, and the shared session asks the agent to explain once. The
   exchange state and the run-settled notification come from `CopilotKitCore`
   (`getActivityExchangeState`, `onActivityRunSettled`) through the same
   `bindMcpApp` options React and Angular pass. The contract is documented once,
   in `packages/mcp-apps-renderer/README.md` ("Failure lifecycle"); the proofs
   per framework are in the matrix below.
10. Both Vue dispatch entry points (the public `useRenderActivityMessage` hook
    and `CopilotChatMessageView`'s own resolver) hand content the schema
    rejected only to a renderer carrying the MCP Apps lifecycle mark, never to a
    custom renderer that merely reuses the public schema.

### MCP Apps lifecycle and failure parity

Each cell names the file and the case that proves the contract in that
framework. React and Vue prove the chat-level rows through the real chat, the
real core run tracking and the real session; Angular proves them on the real
session with a core stub, since it has no chat-level MCP suite. Rows owned by
the shared package are proven once, in `packages/mcp-apps-renderer`.

| Contract                                                                                                                         | React                                                                                                           | Vue                                                                                                                  | Angular                                                                                                    | Status                               |
| -------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| Widget mounted before its result; incomplete fragments keep the instance; decided only when the run ends                         | `MCPAppsActivityRenderer.e2e.test.tsx`, "keeps a provisional fragment mounted, then removes and explains it..." | `MCPAppsActivityRenderer.e2e.test.ts`, same case                                                                     | `mcp-apps-widget.spec.ts`, "keeps a provisional fragment mounted, then removes and explains it..."         | matched                              |
| Tool error or invalid result already settled at bind: removed, never fetched, removed from the store, explained once             | `MCPAppsActivityRenderer.e2e.test.tsx`, "removes a widget whose tool errored once the run settles..."           | `MCPAppsActivityRenderer.e2e.test.ts`, same case                                                                     | `mcp-apps-widget.spec.ts`, "removes the widget and explains a tool error that was already settled at bind" | matched                              |
| Failing `tools/call`: the response reaches the widget while it is still on screen, then one removal and one report               | `MCPAppsProxy.e2e.test.tsx`, "answers a failing tools/call before the widget is removed..."                     | `MCPAppsUiMessage.e2e.test.ts`, same case, in a trailing Vue-specific block (no `MCPAppsProxy.e2e` counterpart file) | `mcp-apps-widget.spec.ts`, "answers a failing tools/call before the frame is removed..."                   | matched (Vue file placement differs) |
| Same identity remounts: stays removed, no new bind; a different resource mounts again with its own session                       | `MCPAppsActivityRenderer.removed.test.tsx`                                                                      | `MCPAppsActivityRendererRemoved.test.ts`                                                                             | `mcp-apps-widget.spec.ts`, "stays removed for the same resource, and mounts again for a different one"     | matched                              |
| Same activity, different resource on the user's next turn: mounts and fetches again, no second report                            | `MCPAppsActivityRenderer.e2e.test.tsx`, "mounts a new widget when the same activity comes back..."              | `MCPAppsActivityRenderer.e2e.test.ts`, same case                                                                     | Widget-level only (row above); Angular has no chat-level MCP suite                                         | partial (Angular)                    |
| Dispatch: only the marked built-in renderer receives content the schema rejected                                                 | `use-render-activity-message.mcp-apps.test.tsx`                                                                 | `use-render-activity-message.mcp-apps.test.ts` and `CopilotChatMessageView.test.ts`, "invalid MCP Apps content"      | `copilot-activity.spec.ts`, the two MCP Apps cases                                                         | matched                              |
| `isError: true` retires and is reported on both paths; history (`untracked`) is removed but never reported                       | shared: `widget-controller.test.ts`                                                                             | shared                                                                                                               | shared                                                                                                     | matched (shared package)             |
| Concurrency: racing commits, remount during a queued or in-flight report, retiring reserved to the proxy path, teardown mid-send | shared: `widget-controller.test.ts`, `session.e2e.test.ts`                                                      | shared                                                                                                               | shared                                                                                                     | matched (shared package)             |
| Isolation: thread switch, two activities on one agent, two instances of one resource, same id across threads                     | shared: `widget-controller.test.ts`, `request-queue.test.ts`                                                    | shared                                                                                                               | shared                                                                                                     | matched (shared package)             |

Known divergence, not a parity gap: the explanation run is acknowledged
immediately in the React and Vue chat harnesses instead of streamed, because
their mock agents never complete their event stream and the shared queue
serializes the explanation with every proxied request of the same agent. The
assertion depth is the same on both sides (one developer message, one run,
store and DOM state).
...
| `MCPAppsActivityRenderer.e2e.test.tsx`                                                             | `MCPAppsActivityRenderer.e2e.test.ts`             | matched  | Strict counterpart now passes with React-matching MCP activity flow coverage (request/loading/error/content and metadata scenarios). Includes the `Failure lifecycle` block, mirrored case for case: terminal at bind, provisional then settled without another update, and a different resource on the next turn without a second report, all through the real chat, core and session (see the lifecycle matrix above).                                                                                                                                                                                                                              |
| `MCPAppsUiMessage.e2e.test.tsx`                                                                    | `MCPAppsUiMessage.e2e.test.ts`                    | partial  | Mirrors `ui/message` continuation semantics (`followUp` default/override behavior plus add-message assertions), but not React's `_meta.copilotkit` extension-channel case nor its thread-change follow-up guard. Marked `partial` per rule 8 rather than left `matched`. Also hosts the Vue `tools/call` failure case (`MCP Apps tools/call failure lifecycle`, a trailing Vue-specific block) ported from React's `MCPAppsProxy.e2e`, which has no Vue counterpart file.                                                                                                                                                                             |
| `MCPAppsActivityRendererSeed.test.tsx`                                                             | `MCPAppsActivityRendererSeed.test.ts`             | matched  | Mirrors the self-subscription wiring: `messageId` is handed to `bindMcpApp`, and an activity absent from the agent store is seeded via `syncContent` right after binding (props that never change afterwards).                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `MCPAppsActivityRenderer.removed.test.tsx`                                                         | `MCPAppsActivityRendererRemoved.test.ts`          | matched  | Mirrors the removal contract of the failure lifecycle: `getExchangeState` and `subscribeRunSettled` are wired from the core, nothing is rendered once `onRemoved` fires, the same identity stays removed with no new bind, and a different resource mounts again with its own session.                                                                                                                                                                                                                                                                                                                                                                |
| `MCPAppsProxy.e2e.test.tsx`                                                                        | _(missing)_                                       | unmapped | `ui/initialize` negotiation, proxy error codes and the open-link policy are proven at the shared-package level and in Angular, not yet through the Vue adapter. Its `tools/call` failure case is the one exception, ported into `MCPAppsUiMessage.e2e.test.ts`.                                                                                                                                                                                                                                                                                                                                                                                       |
| _(no React counterpart yet)_                                                                       | `MCPAppsActivityRendererContentError.test.ts`     | vue-only | Vue-first coverage of the recoverable-content contract: a rejected activity update shows an error that DISAPPEARS once valid content resumes, while a fatal session error stays. React wires the same `onContentError` hook but has no DOM counterpart yet.                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `use-human-in-the-loop.e2e.test.tsx`            | `use-human-in-the-loop.e2e.test.ts` (+ `use-human-in-the-loop.test.ts`)         | matched | Dedicated strict counterpart passes React-mirrored HITL status, interaction, registration, attribution, abort, and reconnection coverage. Vue adds focused lifecycle assertions for already-aborted signals, one-shot abort cleanup, cleanup-before-respond, exhaustive status props, scoped renderer disposal, and preserving a pending promise across renderer unmount. The abort E2E uses the Vue harness's `CopilotKitCore.stopAgent({ agent })` path because its configured agent may be cloned; this is harness-only lifecycle glue. Vue preserves React's no-settle-on-unmount reconnect/remount behavior. |
...
| `use-render-activity-message.mcp-apps.test.tsx` | `use-render-activity-message.mcp-apps.test.ts`                                  | matched | Mirrors the dispatch rule for rejected MCP Apps content: the marked built-in renderer receives the raw content, a custom renderer that merely reuses the public schema is skipped, and valid content still renders. Vue's second dispatch entry point, `CopilotChatMessageView`, is covered by the `CopilotChatMessageView invalid MCP Apps content` block in `CopilotChatMessageView.test.ts` (React has a single entry point).                                                                                                                                                                                  |
